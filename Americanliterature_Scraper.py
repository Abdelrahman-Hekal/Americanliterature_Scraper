...
def scrape_americanliterature(path):

    start = time.time()
    print('-'*75)
    print('Scraping americanliterature.com ...')
    print('-'*75)
    ...
    if path == '':
        name = 'americanliterature_data.xlsx'
        # getting the books under each category
        links = []
        nbooks = 0
        homepages = ['https://americanliterature.com/100-great-short-stories/']

        for homepage in homepages:
            driver.get(homepage)         
            # scraping books urls
            p = wait(driver, 10).until(EC.presence_of_element_located((By.CSS_SELECTOR, "p[style='font-weight: 600;']")))
            titles = wait(p, 10).until(EC.presence_of_all_elements_located((By.TAG_NAME, "a")))
            for title in titles:
                try:
                    ...
                    print('The below error occurred during the scraping from americanliterature.com, retrying ..')
                    print('-'*50)
                    print(err)
                    continue
        ...
        with open('americanliterature_links.csv', 'w', newline='\n', encoding='utf-8') as f:
            writer = csv.writer(f)
    ...
    if path != '':
        ...
        df_links = pd.read_csv('americanliterature_links.csv')
    ...
    for i, link in enumerate(links):
        ...
        try:
            ...
            author, author_link = '', ''
            try:
                tag = wait(driver, 2).until(EC.presence_of_element_located((By.CSS_SELECTOR, "a[itemprop='author']")))
            ...
            rating = ''             
            try:
                rating = wait(driver, 2).until(EC.presence_of_element_located((By.CSS_SELECTOR, "span[id='new_score']"))).get_attribute('textContent').strip()
            except:
                pass     
            ...
            featured, date = '', ''             
            try:
                div = wait(driver, 2).until(EC.presence_of_element_located((By.CSS_SELECTOR, "div[style='font-size: 0.85em; font-style: italic;']")))
                tags = wait(div, 2).until(EC.presence_of_all_elements_located((By.TAG_NAME, "b")))
                if len(tags) == 3:
                    featured = tags[1].get_attribute('textContent').replace('\n', '')
                    date = tags[2].get_attribute('textContent')
            except:
                pass     
                
            details['Featured'] = featured
            details['Featuring Date'] = date
    ...
    print('-'*75)
    print(f'americanliterature.com scraping process completed successfully! Elapsed time {elapsed} mins')
    print('-'*75)
    driver.quit()
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 2:
        path = sys.argv[1]
    data = scrape_americanliterature(path)
